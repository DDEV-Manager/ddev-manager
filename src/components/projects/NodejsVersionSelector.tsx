...
import { useChangeNodejsVersion } from "@/hooks/useDdev";
...
interface NodejsVersionSelectorProps {
...
export function NodejsVersionSelector({
  currentVersion,
  projectName,
  approot,
  disabled = false,
  ...
  const changeNodejsVersion = useChangeNodejsVersion();
  ...
  useEffect(() => {
    ...
    listen<CommandStatus>("command-status", (event) => {
      ...
      if (command !== "change-nodejs" || project !== projectName) return;

      if (status === "finished") {
        toast.success("Node.js version changed", `Node.js version changed to ${selectedVersion}`);
        setIsChanging(false);
        setSelectedVersion(null);
      } else if (status === "error") {
        toast.error("Failed to change Node.js version", "Check the terminal for details");
        setIsChanging(false);
        setSelectedVersion(null);
      } else if (status === "cancelled") {
        toast.info("Operation cancelled", "Node.js version change was cancelled");
        setIsChanging(false);
        setSelectedVersion(null);
      }
    }).then((fn) => {
  ...
  const nodejsVersions = schema?.nodejs_versions ?? [];
      ...
      <div ref={dropdownRef} className="relative inline-block">
        ...
        >
          {isChanging ? (
            ...
            <>
              <span>Node.js {displayVersion}</span>
              {!isDisabled && nodejsVersions.length > 0 && <ChevronDown className="h-3 w-3" />}
            </>
          )}
        </button>

        {isOpen && nodejsVersions.length > 0 && (
